...
<head>
    ...
    <link rel="stylesheet" href="./css/sellbbsdetail.css?afters">
		...
		<div id="mainFrame">
			...
			<div id="upAndDel">
				...
				<div id="contenttitleFrame">
					...
					<div class="contentinfo"><%=cdto.getId() %></div>					
				</div>
					
				<div id="CtitleFrame">
					<div class="writeinfo">제목</div>
					<div class="contentinfo"><%=cdto.getTitle() %></div>
				</div>
				...
				<div id="replyFrame">
					<div id="replytitle">댓글목록</div>
					
					<div id="Rwriteframetop"></div>
					<% if(replyBbs == null || replyBbs.size() == 0){ %>
					<div class="Rwriteframe">
						<div id="Rwriteinfo"></div>
						<div id="replycontent">작성된 댓글이 없습니다.</div>
						<div id="UpDelReply"></div>
					</div>
					...
					%>
						<div class="Rwriteframe">
							<div id="Rwriteinfo"> <!-- 작성자 아이디 -->
							<%=arrow(rpbbs.getDepth()) %> 
							<%=rpbbs.getId()%>
							</div>
							<div id="replycontent"><%=rpbbs.getContent()%></div> <!-- 댓글내용 -->
							<div id="UpDelReply"> <!-- 수정 삭제 답글 -->
								<%if(userinfo.getId().equals( rpbbs.getId() )){%>
								...
								<% } %>
								<a style="cursor:pointer" onclick="sellreplyanswer(<%=rpbbs.getRid()%>)">답글</a>
							</div>
						</div>
